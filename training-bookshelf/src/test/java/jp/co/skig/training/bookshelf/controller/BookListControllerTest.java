...
class BookListControllerTest {
  ...
  void BK01_002_検索条件指定で検索しセッション保存() throws Exception {
    ...
    when(bookService.findAll(eq("Java"), isNull(), isNull(), any(), any(), anyInt(), anyInt()))
  ...
  void BK01_003_セッションから検索条件復元() throws Exception {
    ...
    when(bookService.count(eq("夏目漱石"), isNull(), isNull())).thenReturn(1);
    when(bookService.findAll(eq("夏目漱石"), isNull(), isNull(), any(), any(), anyInt(), anyInt()))
        .thenReturn(createBooks(1));
    when(categoryService.findAll()).thenReturn(Collections.emptyList());
  ...
  void BK01_012_カテゴリのみ指定で検索条件ありと判定() throws Exception {
    ...
    when(bookService.count(isNull(), isNull(), eq(1))).thenReturn(2);
    when(bookService.findAll(isNull(), isNull(), eq(1), any(), any(), anyInt(), anyInt()))
  ...
  void BK01_004_ソート指定あり() throws Exception {
    // Given
    when(bookService.count(any(), any(), any())).thenReturn(0);
    when(bookService.findAll(any(), any(), any(), eq("title"), eq("ASC"), anyInt(), anyInt()))
  ...
  void BK01_007_page指定が下限未満は1に補正() throws Exception {
    // Given
    when(bookService.count(any(), any(), any())).thenReturn(20);
    when(bookService.findAll(any(), any(), any(), any(), any(), anyInt(), anyInt()))
        .thenReturn(createBooks(20));
    when(categoryService.findAll()).thenReturn(Collections.emptyList());
  ...
  void BK01_010_検索条件ありで検索結果メッセージ表示() throws Exception {
    // Given
    when(bookService.count(isNull(), eq("夏目"), isNull())).thenReturn(5);
    when(bookService.findAll(isNull(), eq("夏目"), isNull(), any(), any(), anyInt(), anyInt()))
